...
const JarvisExperience: React.FC = () => {
    ...
    const generateAnalysis = async () => {
        try {
            ...
            const prompt = `Você é um Estrategista de Vendas e Branding Sênior da Bicalho Ads Lab. 
      Um cliente chamado ${userName} acabou de submeter o instagram ${instagramUrl} para avaliação.
      Gere um diagnóstico curto, muito persuasivo e direto ao ponto (em tom de áudio/locução, sem markdown). 
      Diga que analisou a arquitetura do negócio e identifique de forma genérica, mas impactante, 3 falhas:
      1) Falta de autoridade/posicionamento.
      2) Quebras de persuasão na estrutura de conversão.
      3) Estratégia de tráfego com ângulos errados.
      Termine encorajando-o a aplicar o plano estratégico para escalar imediatamente. Máximo de 70 palavras. Seja implacável e profissional.`;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!userName || !instagramUrl) return;
    ...
    const playTTS = (text: string) => {
        ...
        // Cancela leituras anteriores pra não encavalar
        window.speechSynthesis.cancel();

        const utterance = new SpeechSynthesisUtterance(text);
        utterance.lang = 'pt-BR';
        // Parametros para deixar a voz Web Nativa um pouco mais séria e profunda
        utterance.rate = 1.0;
        utterance.pitch = 0.9;
